<div class="container-fluid services">

<div class="container">
    <div class="col-md-4 details animated zoomIn ">
        <img class="img img-responsive center" alt="" src="{{asset('bootstrap/images/home/details/1.jpg')}}">
        <H3>SCUBA DIVING</H3>
        <hr class="hr-large">
        <p>The depths of the Indian Ocean hold as many mysteries as the land above it. Venture into the dark corners of the ocean floor and witness moments of history — reef and wreckage alike — that have been swallowed whole into a life aquatic.</p>

    </div>

    <div class="col-md-4 details animated zoomIn ">
        <img class="img img-responsive center" alt="" src="{{asset('bootstrap/images/home/details/2.jpg')}}">
         <H3>LUXURY ROOMS</H3>
        <hr class="hr-large">
        <p >At Aminva Villa, our vibrant collection of Superior & Deluxe Rooms, Standard Single and Standard Double  combine the bohemian with the luxurious to create a distinct accommodation experience at our Hikkaduwa beach hotel.each room also promises an incomparable view of the vast ocean beyond.</p>
    </div>


    <div class="col-md-4 details animated zoomIn ">
        <img class="img img-responsive center" alt="" src="{{asset('bootstrap/images/home/details/3.jpg')}}">
         <H3>BEST FOR DINING</H3>
        <hr class="hr-large">
        <p>With its panoramic views of the Indian Ocean in a defined casual-chic setting, Sea Spray introduces an exclusive alfresco dining experience to Hikkaduwa. Inspired by European seafronts, the atmosphere is refined but relaxed, with seamless understated service.</p>
    </div>

       
    </div>
</div>